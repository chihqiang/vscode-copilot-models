...
suite("TokenPlan Test Suite", () => {
  ...
  suite("extractHostname", () => {
    test("extracts hostname from full URL", () => {
      assert.strictEqual(plan.extractHostname("https://api.deepseek.com/v1/chat"), "api.deepseek.com");
    });

    test("extracts hostname without port", () => {
      assert.strictEqual(plan.extractHostname("https://dashscope.aliyuncs.com:443"), "dashscope.aliyuncs.com");
    });
  ...
  suite("detectProviderFromUrl", () => {
    test("detects Qwen by defaultBaseUrl hostname", () => {
      const preset = plan.detectProviderFromUrl("https://token-plan.cn-beijing.maas.aliyuncs.com/compatible-mode/v1/chat");
      assert.ok(preset);
      assert.strictEqual(preset!.id, "qwen");
    });

    test("detects Qwen by subdomain of defaultBaseUrl", () => {
      const preset = plan.detectProviderFromUrl("https://sub.token-plan.cn-beijing.maas.aliyuncs.com/v1");
      assert.ok(preset);
      assert.strictEqual(preset!.id, "qwen");
    });

    test("returns undefined for unknown URL", () => {
      assert.strictEqual(plan.detectProviderFromUrl("https://unknown.example.com"), undefined);
    });

    test("rejects spoofed hostname", () => {
      assert.strictEqual(plan.detectProviderFromUrl("https://evil-token-plan.cn-beijing.maas.aliyuncs.com.attacker.com"), undefined);
    });

    test("does not detect unrelated hostname (e.g. dashscope)", () => {
      assert.strictEqual(plan.detectProviderFromUrl("https://dashscope.aliyuncs.com/v1"), undefined);
    });
  });
  ...
  suite("Plan CRUD", () => {
    ...
    test("storePlan adds new plan", async () => {
      await plan.storePlan({
        planId: "plan-1", planName: "P1", baseUrl: "https://a.com",
        providerId: "a", models: [], createdAt: 1, updatedAt: 1,
      });
    ...
    test("storePlan updates existing plan", async () => {
      ...
      await plan.storePlan({
        planId: "plan-1", planName: "Updated", baseUrl: "https://a.com",
        providerId: "a", models: [], createdAt: 1, updatedAt: 2,
      });
      assert.strictEqual(plan.getPlans().length, 1);
    ...
    test("removePlan removes by ID", async () => {
      await plan.storePlan({
        planId: "keep", planName: "K", baseUrl: "https://a.com",
        providerId: "a", models: [], createdAt: 1, updatedAt: 1,
      });
      await plan.storePlan({
        planId: "remove", planName: "R", baseUrl: "https://b.com",
        providerId: "b", models: [], createdAt: 2, updatedAt: 2,
      });
    ...
    test("getPlanModelIds returns all covered model IDs", async () => {
      await plan.storePlan({
        planId: "p1", planName: "P1", baseUrl: "https://a.com",
        models: [{ id: "m1" }, { id: "m2" }],
  ...
  suite("Consumption", () => {
    ...
    test("recordConsumption appends", async () => {
      await plan.recordConsumption({
        planId: "p1", modelId: "m1",
        promptTokens: 10, completionTokens: 5, totalTokens: 15, timestamp: 1,
      });
      await plan.recordConsumption({
        planId: "p1", modelId: "m2",
        promptTokens: 20, completionTokens: 10, totalTokens: 30, timestamp: 2,
      });
  ...
  suite("resolvePlanOverride", () => {
    ...
    test("returns undefined when model not covered", async () => {
      await plan.storePlan({
        planId: "p1", planName: "P1", baseUrl: "https://a.com",
        models: [{ id: "m1" }], createdAt: 1, updatedAt: 1,
      });
