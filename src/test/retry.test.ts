...
suite('CircuitBreaker Test Suite', () => {
  ...
  test('opens after reaching failure threshold', async () => {
    const cb = new CircuitBreaker({ failureThreshold: 2, resetTimeoutMs: 50000 });
    const failingFn = async () => { throw new Error('fail'); };
  ...
  test('throws CircuitBreakerError when OPEN', async () => {
    const cb = new CircuitBreaker({ failureThreshold: 1, resetTimeoutMs: 50000 });
    const failingFn = async () => { throw new Error('fail'); };
...
suite('calculateDelay Test Suite', () => {
  ...
  test('increases exponentially with attempt', () => {
    const delay0 = calculateDelay(0, { baseDelayMs: 1000, maxDelayMs: 100000, jitterMs: 0 });
    const delay1 = calculateDelay(1, { baseDelayMs: 1000, maxDelayMs: 100000, jitterMs: 0 });
    const delay2 = calculateDelay(2, { baseDelayMs: 1000, maxDelayMs: 100000, jitterMs: 0 });

    assert.ok(delay1 >= delay0 * 1.5, `delay1=${delay1} should be >= delay0*1.5=${delay0 * 1.5}`);
    assert.ok(delay2 >= delay1 * 1.5, `delay2=${delay2} should be >= delay1*1.5=${delay1 * 1.5}`);
  });

  test('caps at maxDelayMs', () => {
    const delay100 = calculateDelay(100, { baseDelayMs: 1000, maxDelayMs: 5000, jitterMs: 0 });
    assert.ok(delay100 <= 5000, `Expected <= 5000, got ${delay100}`);
  });

  test('uses custom config', () => {
    const d = calculateDelay(0, { baseDelayMs: 500, maxDelayMs: 10000, jitterMs: 0 });
    assert.ok(d >= 500 && d <= 500);
  });

  test('adds jitter', () => {
    const delays = Array.from({ length: 20 }, () => calculateDelay(0, { baseDelayMs: 1000, maxDelayMs: 5000, jitterMs: 500 }));
